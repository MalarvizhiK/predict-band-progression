...
cos_service = json.loads(os.environ['VCAP_SERVICES'])['cloud-object-storage'][0]
credentials = cos_service['credentials']
...
class algorithm:
    ...
    def cluster_dataset(excel_filename):
        ...
        if os.path.isfile(excel_filename):
            ...
            try:
                res=cos.download_file(Bucket='malark', Key=excel_filename, Filename=excel_filename)
    ...
    def predict_progression(final):
        ...
        excel_filename = 'SampleProgressionData.xlsx'
        if os.path.isfile(excel_filename):
            print ("File exist")
        else:
            print ("File not exist")
            try:
                ...
                print('File Downloaded')
        dataset = read_excel(excel_filename)
        """
